...
export default class JigsawGenerator {
  ...
  generate(dim): JigsawGenerator {
    ...
    while (true) {
      ...
      if (!pick) {
        console.log('something went wrong', [...this.grid], cell, randomInGrid, randomInOptions, stopIndex);
        this.generate(dim);
        return this;
      }
  ...
  toArrayOfJigsawType(): { type: JigsawPieceType; key: number }[] {
    return this.grid.map((x, i) => {
      return { type: x.options[0], key: i };
    });
  }
}
